...
const MyBookings = () => {
  ...
  const handlePayment=async(bookingId)=>{
    try{
      const {data}=await axiosInstance.post("/api/bookings/stripe-payment",{
        bookingId
      });
      if(data.success){
                    ...
                    <div className="col-span-1 md:col-span-2">
                     <div className='space-y-2'>
                            ...
                            booking.isPaid? "bg-green-100 text-green-700":"bg-red-100 text-red-700"}`}>
                             <button className="cursor-pointer"  onClick={()=>handlePayment(booking._id)}>{booking.isPaid? "Paid":"Pay now"}</button>
                            </div>

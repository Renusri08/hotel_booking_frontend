...
const SingleRoom = () => {
  const {roomData,axiosInstance,navigate}=useContext(AppContext);
  ...
  const checkRoomAvailability=async()=>{
    try{
      ...
      if(data.success){
        if(data.isAvailable){
          setIsAvailable(true);
          toast.success("Room is avilable for booking");
        }
      }
      else{
  ...
  const onSubmitHandler=async(e)=>{
    ...
    try{
      if(!isAvailable){
        ...
        const {data}=await axiosInstance.post("/api/bookings/book",{
          room:room._id,
          checkInDate:bookingData.checkIn,
          checkOutDate:bookingData.checkOut,
        ...
        <div className='grid lg:grid-cols-3 gap-8'>
          <div className='lg:col-span-2 space-y-8'>
            <div className='bg-white rounded-2xl shadow-lg p-8'>
                <h2 className='text-2xl font-bold text-gray-800'>About this Room</h2>
                <p className='text-gray-600 leading-relaxed mt-3'>{room.description}</p>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className='bg-white rounded-2xl shadow-lg p-8 sticky top-8'>
                <h2 className="text-exl font-bold text-gray-800 mb-6">
                  Book This Room
                </h2>
                <form onSubmit={onSubmitHandler} className="space-y-4">
                  <div>
                    ...
                    <input type="date"name='checkIn' min={new Date().toISOString().split("T")[0]} value={bookingData.checkIn} onChange={onChangeHandler}
                  ...
                  <div>
                    ...
                    <input type="date" name="checkOut" min={new Date().toISOString().split("T")[0]} vlaue={bookingData.checkOut} onChange={onChangeHandler}
                    className='w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent'/>
                  </div>
                  <div>
                    <label htmlFor="" className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <input type="number" name='persons' vlaue={bookingData.persons} onChange={onChangeHandler}
                    className='w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent'/>
                  </div>
                  ...
                  <button 
                  className={`w-full py-3 px-6 rounded-lg font-medium transition-all duration-200 bg-blue-600 hover:bg-blue-700 text-white cursor-pointer`}
                  type="submit" 
                  >
                   {isAvailable ? "Book Now" : "check Availability"}</button>
                </form>
            </div>
          </div>
        </div>
